/*
 * FREENOVE HUB - FULL CODE
 * Fixed: ESP-NOW channel locked to 11 (eduroam channel)
 * Fixed: OnDataRecv uses correct esp32 core 3.x callback signature
 */
...
const float FLOW_RATE = 0.051;      // L/s
const float TARGET_TOTAL_L = 4.89;
...
void sendToMQTT() {
  if (client.connected()) {
    ...
    Serial.println("MQTT Published: " + p);
  }
}
...
void OnDataRecv(const esp_now_recv_info_t *recv_info, const uint8_t *incomingData, int len) {
  ...
  Serial.printf("✅ Received from Board %d | Raw: %d | Moisture: %d%% | Batt: %.2fV\n",
    incoming.id, incoming.moistureRaw, incoming.soilmoisture, incoming.batteryVoltage);
  ...
  if (!stormMode) {
    digitalWrite(PUMP_A, currentPcA < threshold ? PUMP_ON : PUMP_OFF);
    digitalWrite(PUMP_B, currentPcB < threshold ? PUMP_ON : PUMP_OFF);
  }
  newDataReady = true;
}
...
void setup() {
  ...
  // Init ESP-NOW and lock to channel 1 (eduroam channel)
  esp_now_init();
  ...
  server.on("/set_base", []() { baseAdcA = currentAdcA; baseAdcB = currentAdcB; server.send(200, "text/plain", "Set"); });
  server.on("/start_storm", []() { stormMode = true; currentTotalL = 0; cycleStartTime = millis(); server.send(200, "text/plain", "Start"); });
  server.on("/stop_all", []() { stormMode = false; digitalWrite(PUMP_A, PUMP_OFF); digitalWrite(PUMP_B, PUMP_OFF); server.send(200, "text/plain", "Stop"); });
  server.begin();
}
