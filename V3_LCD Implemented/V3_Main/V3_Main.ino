...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  // CHANGE MADE
  // Store data in the correct array slot (Board 1 -> index 0)
  int index = incomingReadings.id - 1;
  if (index >= 0 && index < 4) {
    boardsStruct[index] = incomingReadings;
    Serial.printf("Updated Board %d data\n", incomingReadings.id);
  }

  // UPDATE LCD DISPLAY
  lcd.clear();
  lcd.setCursor(0, 0);
  // Row 1: Display Board ID and Voltage
  lcd.printf("Node:%d V:%.2fV", incomingReadings.id, incomingReadings.batteryVoltage);
  
  lcd.setCursor(0, 1);
  // Row 2: Display Soil Moisture
  lcd.printf("Moisture: %d%%", incomingReadings.soilmoisture);
