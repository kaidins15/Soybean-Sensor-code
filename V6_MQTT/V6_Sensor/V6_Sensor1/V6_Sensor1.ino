...
typedef struct struct_message {
  int id;
  float temp; // not using this
  float hum; // not using this
  int battery; // not using this
  float batteryVoltage; // not using this
  int moistureRaw;
  int soilmoisture;
} struct_message;
...
void loop() {
  ...
  float x = (float)moistureRaw;
  float y = (-0.0992*x) + 231.4;
  int soilmoisture = (int)y;

  //replace with calibration equation
//float x = (float)moistureRaw;
//float y = (-0.00002 * x * x) - (0.0555 * x) + 55.059;
...
Serial.println("%");
  ...
  // CHANGE MADE
  // Deep Sleep Mode
  // 8. CRITICAL: Wait for transmission to finish before cutting power
 delay(200);
 ...
 // 9. GO TO SLEEP
  ...
  // uint64_t sleep_duration = 60 * 1,000,000;
  uint64_t sleep_duration = 3600000000;
